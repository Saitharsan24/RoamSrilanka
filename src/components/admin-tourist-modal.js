...
function Modal_tourist({closeModal,userId}){
  ...
  const axiosInstance = axios.create({
   ...
   const [touristdetailstatus, setTouriststatus] = useState([]);
   const handleUpdate = () => {
    ...
    axiosInstance
      ...
      .then((response) => {
       ...
       //  console.log(currentStatus)
        ...
        axiosInstance
          .put(`/updateTourist/${userId}`, { status: newStatus })
          .then((response) => {
            // Handle success, close the modal
            closeModal();
            setTouriststatus(response.data.status);
        ...
        <div className="modalBackground-planner">
            <div className="modalContainer-planner">
               <div className="title-modalCloseBtn-planner w-75 mt-1 " onClick={()=>closeModal(false)}> <button  >X</button></div>
                ...
                <div className="body-modal-planner "><p>Do you want to {touristdetailstatus===null?("Enable"):("Disable")}  Account?</p></div>
